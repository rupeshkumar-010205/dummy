...
class DetectionTrainer:
            ...
            for i, batch in pbar:
                ...
                if ni <= nw:
                    ...
                    for j, x in enumerate(self.optimizer.param_groups):
                        ...
                        # this is a strategy I came up with early on since bias is more stable than weights and can tolerate higher LRs without losing stability,
                        # but it's been a while since I experimented with warmup strategies, so if you find a better warmup implementation in your experiments please let us know!
                        x["lr"] = np.interp(
    ...
    def save_model(self):
        ...
        import pandas as pd  # scope for faster 'import  '
    ...
    def _setup_train(self, world_size):
        ...
        if self.amp and RANK in {-1, 0}:  # Single-GPU and DDP
            ...
            self.amp = torch.tensor(self.check_amp(model=self.model), device=self.device)
    ...
    def check_amp(self,model):
        ...
        def amp_allclose(m, im):
            ...
            p = next(m.parameters())  # for device, type
            im = torch.zeros((1, 3, *imgsz), device=p.device).cuda() # input image (must be zeros, not empty)
            m.eval()
            a = m(im)
            with torch.cuda.amp.autocast(True):
                b = m(im) # AMP inference
            del m
            return a[0].shape == b[0].shape and torch.allclose(a[0], b[0].float(), atol=0.5)  # close to 0.5 absolute tolerance
        ...
        try:
            ...
            LOGGER.warning(
                ...
                f"Unable to load YOLOv8n due to possible   package modifications. {warning_msg}"
            )
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        if name in {"Adam", "Adamax", "AdamW", "NAdam", "RAdam"}:
            ...
            raise NotImplementedError(
                ...
                "To request support for addition optimizers please visit https://github.com/ / ."
            )
